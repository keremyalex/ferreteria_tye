        ...
        <SidebarLayout>
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-5">
                            ...
                            <div class="relative md:col-span-2">
                                <input
                                    ...
                                    class="block w-full border-gray-300 rounded-md shadow-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                />
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-700">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-300">
                                            <button @click="sort('cantidad_actual')" class="flex items-center space-x-1 hover:text-gray-700 dark:hover:text-gray-100">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                                    <tr v-for="item in inventoryItems.data" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0 w-10 h-10">
                                                    <div class="flex items-center justify-center w-10 h-10 bg-indigo-100 rounded-lg dark:bg-indigo-800">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium" :class="{
                                                'text-red-600 dark:text-red-400': parseFloat(item.cantidad_actual) === 0,
                                                'text-yellow-600 dark:text-yellow-400': parseFloat(item.cantidad_actual) > 0 && parseFloat(item.cantidad_actual) <= parseFloat(item.cantidad_minima),
                                                'text-green-600 dark:text-green-400': parseFloat(item.cantidad_actual) > parseFloat(item.cantidad_minima)
                                            }">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900 dark:text-white">
                                                Bs {{ Number(item.precio_venta || item.product.precio_venta).toLocaleString('es-BO', { minimumFractionDigits: 2 }) }}
                                            </div>
                                            <div v-if="item.precio_venta" class="text-xs text-blue-600 dark:text-blue-400">
                                                Precio específico
                                            </div>
                                            <div v-else class="text-xs text-gray-500 dark:text-gray-400">
                                                Base: Bs {{ Number(item.product.precio_venta).toLocaleString('es-BO', { minimumFractionDigits: 2 }) }}
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span v-if="parseFloat(item.cantidad_actual) === 0" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200">
                                                Sin Stock
                                            </span>
                                            <span v-else-if="parseFloat(item.cantidad_actual) <= parseFloat(item.cantidad_minima)" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200">
                                                Stock Bajo
                                            </span>
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    <span class="text-sm text-gray-700 dark:text-gray-300">
                                        Mostrando {{ inventoryItems.from }} a {{ inventoryItems.to }} de {{ inventoryItems.total }} resultados
                                    </span>
                                </div>
                                <div class="flex space-x-2">
                                    <template v-for="link in inventoryItems.links" :key="link.label">
                                        <Link 
                                            v-if="link.url"
                                            :href="link.url"
                                            v-html="link.label"
                                        ...
                                        <span 
                                            v-else
                                            v-html="link.label"
                                            class="px-3 py-2 text-sm font-medium text-gray-400 bg-gray-100 rounded-md cursor-not-allowed dark:bg-gray-700 dark:text-gray-500"
                                        />
                                    </template>
                                </div>
                            </div>
            ...
            <div v-if="showUpdateModal" class="fixed inset-0 z-50 overflow-y-auto">
                <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                    <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" @click="closeUpdateModal"></div>
                                    ...
                                    <div class="grid grid-cols-1 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div class="mb-1 text-xs text-gray-500 dark:text-gray-400">
                                                Precio base del producto: Bs {{ updateForm.product?.precio_venta || '0.00' }}
                                            </div>
                                            <input
                                                v-model="updateForm.precio_venta_override"
                                                type="number"
                                                step="0.01"
                                                min="0"
                                            ...
                                            <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                                Si se especifica, este precio tendrá prioridad sobre el precio base
                                            </div>
                                        </div>
                                    </div>
...
const openUpdateModal = (item) => {
    ...
    updateForm.precio_venta_override = item.precio_venta
    updateForm.product = item.product
